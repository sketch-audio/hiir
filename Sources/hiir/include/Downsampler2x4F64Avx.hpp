/*****************************************************************************
        ...
        Downsampler2x4F64Avx.hpp
        Port of Downsampler2x4Sse.hpp from float to double by Dario Mambro
        Downsampler2x4Sse.hpp by Laurent de Soras, 2015
...
#if ! defined (hiir_Downsampler2x4F64Avx_CODEHEADER_INCLUDED)
#define hiir_Downsampler2x4F64Avx_CODEHEADER_INCLUDED
...
__m256d	Downsampler2x4F64Avx <NC>::process_sample (const double in_ptr [_nbr_chn * 2]) noexcept
{
	...
	const __m256d   in_0 = _mm256_loadu_pd (in_ptr           );
	const __m256d   in_1 = _mm256_loadu_pd (in_ptr + _nbr_chn);
...
__m256d	Downsampler2x4F64Avx <NC>::process_sample (__m256d in_0, __m256d in_1) noexcept
{
	...
	const __m256d   sum = _mm256_add_pd (spl_0, spl_1);
	const __m256d   out = _mm256_mul_pd (sum, _mm256_set1_pd (0.5f));
...
void	Downsampler2x4F64Avx <NC>::process_block (double out_ptr [], const double in_ptr [], long nbr_spl) noexcept
{
	...
	do
	{
		const __m256d   val = process_sample (in_ptr + pos * (_nbr_chn * 2));
		_mm256_storeu_pd (out_ptr + pos * _nbr_chn, val);
		++ pos;
	}
	while (pos < nbr_spl);
}
...
void	Downsampler2x4F64Avx <NC>::process_sample_split (__m256d &low, __m256d &high, __m256d in_0, __m256d in_1) noexcept
{
	...
	const __m256d   sum = _mm256_add_pd (spl_0, spl_1);
	low  = _mm256_mul_pd (sum, _mm256_set1_pd (0.5f));
	high = _mm256_sub_pd (spl_0, low);
}
...
void	Downsampler2x4F64Avx <NC>::process_block_split (double out_l_ptr [], double out_h_ptr [], const double in_ptr [], long nbr_spl) noexcept
{
	...
	do
	{
		...
		_mm256_storeu_pd (out_l_ptr + pos * _nbr_chn, low);
		_mm256_storeu_pd (out_h_ptr + pos * _nbr_chn, high);
		++ pos;
	}
	while (pos < nbr_spl);
}
...
#endif   // hiir_Downsampler2x4F64Avx_CODEHEADER_INCLUDED
