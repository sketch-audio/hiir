/*****************************************************************************
        ...
        Downsampler2x8Avx.hpp
        Ported  Downsampler2x4Sse.hpp from SSE to AVX by Dario Mambro
        Downsampler2x4Sse.hpp by Laurent de Soras
...
#if ! defined (hiir_Downsampler2x8Avx_CODEHEADER_INCLUDED)
#define hiir_Downsampler2x8Avx_CODEHEADER_INCLUDED
...
template <int NC>
constexpr int 	Downsampler2x8Avx <NC>::_nbr_chn;
template <int NC>
constexpr int 	Downsampler2x8Avx <NC>::NBR_COEFS;
template <int NC>
constexpr double	Downsampler2x8Avx <NC>::_delay;
...
template <int NC>
Downsampler2x8Avx <NC>::Downsampler2x8Avx () noexcept
:	_filter ()
{
...
template <int NC>
void	Downsampler2x8Avx <NC>::set_coefs (const double coef_arr []) noexcept
{
...
template <int NC>
__m256	Downsampler2x8Avx <NC>::process_sample (const float in_ptr [_nbr_chn * 2]) noexcept
{
	assert (in_ptr != nullptr);

	const __m256   in_0 = _mm256_loadu_ps (in_ptr           );
	const __m256   in_1 = _mm256_loadu_ps (in_ptr + _nbr_chn);
...
template <int NC>
__m256	Downsampler2x8Avx <NC>::process_sample (__m256 in_0, __m256 in_1) noexcept
{
	__m256         spl_0 = in_1;
	__m256         spl_1 = in_0;
	...
	const __m256   sum = _mm256_add_ps (spl_0, spl_1);
	const __m256   out = _mm256_mul_ps (sum, _mm256_set1_ps (0.5f));
...
template <int NC>
void	Downsampler2x8Avx <NC>::process_block (float out_ptr [], const float in_ptr [], long nbr_spl) noexcept
{
	assert (in_ptr  != nullptr);
	...
	do
	{
		const __m256   val = process_sample (in_ptr + pos * (_nbr_chn * 2));
		_mm256_storeu_ps (out_ptr + pos * _nbr_chn, val);
		++ pos;
	}
	while (pos < nbr_spl);
}
...
template <int NC>
void	Downsampler2x8Avx <NC>::process_sample_split (__m256 &low, __m256 &high, const float in_ptr [_nbr_chn * 2]) noexcept
{
	assert (in_ptr != nullptr);
...
template <int NC>
void	Downsampler2x8Avx <NC>::process_sample_split (__m256 &low, __m256 &high, __m256 in_0, __m256 in_1) noexcept
{
	__m256         spl_0 = in_1;
	__m256         spl_1 = in_0;
	...
	const __m256   sum = _mm256_add_ps (spl_0, spl_1);
	low  = _mm256_mul_ps (sum, _mm256_set1_ps (0.5f));
	high = _mm256_sub_ps (spl_0, low);
}
...
template <int NC>
void	Downsampler2x8Avx <NC>::process_block_split (float out_l_ptr [], float out_h_ptr [], const float in_ptr [], long nbr_spl) noexcept
{
	assert (in_ptr    != nullptr);
	...
	do
	{
		...
		_mm256_storeu_ps (out_l_ptr + pos * _nbr_chn, low);
		_mm256_storeu_ps (out_h_ptr + pos * _nbr_chn, high);
		++ pos;
	}
	while (pos < nbr_spl);
}
...
template <int NC>
void	Downsampler2x8Avx <NC>::clear_buffers () noexcept
{
	for (int i = 0; i < NBR_COEFS + 2; ++i)
	{
...
#endif   // hiir_Downsampler2x8Avx_CODEHEADER_INCLUDED
