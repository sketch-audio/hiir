...
#if ! defined (hiir_HalfBand2F64Sse2_CODEHEADER_INCLUDED)
#define hiir_HalfBand2F64Sse2_CODEHEADER_INCLUDED
...
template <int NC>
constexpr int 	HalfBand2F64Sse2 <NC>::_nbr_chn;
template <int NC>
constexpr int 	HalfBand2F64Sse2 <NC>::NBR_COEFS;
template <int NC>
constexpr double	HalfBand2F64Sse2 <NC>::_delay;
...
template <int NC>
HalfBand2F64Sse2 <NC>::HalfBand2F64Sse2 () noexcept
:	_bifilter ()
,	_phase (0)
{
	for (int i = 0; i < NBR_COEFS + 2; ++i)
	{
		_mm_store_pd (_bifilter [0] [i]._coef, _mm_setzero_pd ());
		_mm_store_pd (_bifilter [1] [i]._coef, _mm_setzero_pd ());
	}
...
template <int NC>
void	HalfBand2F64Sse2 <NC>::set_coefs (const double coef_arr []) noexcept
{
	assert (coef_arr != nullptr);

	for (int i = 0; i < NBR_COEFS; ++i)
	{
		const auto     c = _mm_set1_pd (DataType (coef_arr [i]));
		_mm_store_pd (_bifilter [0] [i + 2]._coef, c);
		_mm_store_pd (_bifilter [1] [i + 2]._coef, c);
	}
}
...
template <int NC>
__m128d	HalfBand2F64Sse2 <NC>::process_sample (__m128d input) noexcept
{
	const auto     path_arr = process_2_paths (input);
	const auto     low      = _mm_add_pd (path_arr._e, path_arr._o);
...
template <int NC>
void	HalfBand2F64Sse2 <NC>::process_block (double out_ptr [], const double in_ptr [], long nbr_spl) noexcept
{
...
template <int NC>
__m128d	HalfBand2F64Sse2 <NC>::process_sample_hpf (__m128d input) noexcept
{
	const auto     path_arr = process_2_paths (input);
	const auto     high     = _mm_sub_pd (path_arr._e, path_arr._o);
...
template <int NC>
void	HalfBand2F64Sse2 <NC>::process_block_hpf (double out_ptr [], const double in_ptr [], long nbr_spl) noexcept
{
	assert (out_ptr != nullptr);
	assert (out_ptr <= in_ptr || out_ptr >= in_ptr + nbr_spl * _nbr_chn);
...
template <int NC>
void	HalfBand2F64Sse2 <NC>::process_sample_split (__m128d &low, __m128d &high, __m128d input) noexcept
{
	const auto     path_arr = process_2_paths (input);
	low  = _mm_add_pd (path_arr._e, path_arr._o);
	high = _mm_sub_pd (path_arr._e, path_arr._o);
}
...
template <int NC>
void	HalfBand2F64Sse2 <NC>::process_block_split (double out_l_ptr [], double out_h_ptr [], const double in_ptr [], long nbr_spl) noexcept
{
...
template <int NC>
void	HalfBand2F64Sse2 <NC>::clear_buffers () noexcept
{
	_phase = 0;
	for (int i = 0; i < NBR_COEFS + 2; ++i)
	{
		_mm_store_pd (_bifilter [0] [i]._mem, _mm_setzero_pd ());
		_mm_store_pd (_bifilter [1] [i]._mem, _mm_setzero_pd ());
	}
	_mm_store_pd (_prev, _mm_setzero_pd ());
}
...
template <int NC>
constexpr int	HalfBand2F64Sse2 <NC>::_nbr_phases;
...
template <int NC>
typename HalfBand2F64Sse2 <NC>::EvenOdd	HalfBand2F64Sse2 <NC>::process_2_paths (__m128d input) noexcept
{
	const auto     half = _mm_set1_pd (0.5f);
	input = _mm_mul_pd (input, half);

	auto           path_arr = EvenOdd { input, _mm_load_pd (_prev) };

	StageProc2F64Sse2 <NBR_COEFS>::process_sample_pos (
		NBR_COEFS, path_arr._e, path_arr._o, &_bifilter [_phase] [0]
	);

	_mm_store_pd (_prev, input);
	_phase = 1 - _phase;
...
void	HalfBand2F64Sse2 <NC>::process_block_2_paths (double out_l_ptr [], double out_h_ptr [], const double in_ptr [], long nbr_spl, FL fnc_l, FH fnc_h) noexcept
{
	...
	if (_phase == 1)
	{
		const auto     path_arr = process_2_paths (_mm_loadu_pd (in_ptr));
	...
	auto           prev = _mm_load_pd (_prev);
	while (pos < end)
	{
		const auto     ofs_0   = pos * _nbr_chn;
		const auto     ofs_1   = ofs_0 + _nbr_chn;
		auto           input_0 = _mm_loadu_pd (in_ptr + ofs_0);
		auto           input_1 = _mm_loadu_pd (in_ptr + ofs_1);

		input_0 = _mm_mul_pd (input_0, half);
		input_1 = _mm_mul_pd (input_1, half);
	...
	_mm_store_pd (_prev, prev);

	if (pos < nbr_spl)
	{
		assert (pos + 1 == nbr_spl);
		const auto     ofs      = pos * _nbr_chn;
		const auto     path_arr = process_2_paths (_mm_loadu_pd (in_ptr + ofs));
...
template <int NC>
void	HalfBand2F64Sse2 <NC>::store_low (double *ptr, __m128d tmp_0, __m128d tmp_1) noexcept
{
	const auto     low = _mm_add_pd (tmp_0, tmp_1);
...
template <int NC>
void	HalfBand2F64Sse2 <NC>::store_high (double *ptr, __m128d tmp_0, __m128d tmp_1) noexcept
{
	const auto     high = _mm_sub_pd (tmp_0, tmp_1);
...
#endif   // hiir_HalfBand2F64Sse2_CODEHEADER_INCLUDED
