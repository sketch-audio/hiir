...
#if ! defined (hiir_Downsampler2x16Avx512_CODEHEADER_INCLUDED)
#define hiir_Downsampler2x16Avx512_CODEHEADER_INCLUDED
...
template <int NC>
constexpr int 	Downsampler2x16Avx512 <NC>::_nbr_chn;
template <int NC>
constexpr int 	Downsampler2x16Avx512 <NC>::NBR_COEFS;
template <int NC>
constexpr double	Downsampler2x16Avx512 <NC>::_delay;
...
Name: process_sample
Description:
	Downsamples (x2) one pair of vector of 16 samples, to generate one output
	vector.
Input parameters:
...
__m512	Downsampler2x16Avx512 <NC>::process_sample (const float in_ptr [_nbr_chn * 2]) noexcept
{
	...
	const __m512   in_0 = _mm512_loadu_ps (in_ptr           );
	const __m512   in_1 = _mm512_loadu_ps (in_ptr + _nbr_chn);
...
__m512	Downsampler2x16Avx512 <NC>::process_sample (__m512 in_0, __m512 in_1) noexcept
{
	...
	const __m512   sum = _mm512_add_ps (spl_0, spl_1);
	const __m512   out = _mm512_mul_ps (sum, _mm512_set1_ps (0.5f));
...
Name: process_block
Description:
	Downsamples (x2) a block of vectors of 16 samples.
...
void	Downsampler2x16Avx512 <NC>::process_block (float out_ptr [], const float in_ptr [], long nbr_spl) noexcept
{
	...
	do
	{
		const __m512   val = process_sample (in_ptr + pos * (_nbr_chn * 2));
		_mm512_storeu_ps (out_ptr + pos * _nbr_chn, val);
		++ pos;
	}
	while (pos < nbr_spl);
}
...
Description:
	Split (spectrum-wise) in half a pair of vector of 16 samples. The lower
	part of the spectrum is a classic downsampling, equivalent to the output of
	process_sample().
...
void	Downsampler2x16Avx512 <NC>::process_sample_split (__m512 &low, __m512 &high, __m512 in_0, __m512 in_1) noexcept
{
	...
	const __m512   sum = _mm512_add_ps (spl_0, spl_1);
	low  = _mm512_mul_ps (sum, _mm512_set1_ps (0.5f));
	high = _mm512_sub_ps (spl_0, low);
}
...
void	Downsampler2x16Avx512 <NC>::process_block_split (float out_l_ptr [], float out_h_ptr [], const float in_ptr [], long nbr_spl) noexcept
{
	...
	do
	{
		...
		_mm512_storeu_ps (out_l_ptr + pos * _nbr_chn, low);
		_mm512_storeu_ps (out_h_ptr + pos * _nbr_chn, high);
		++ pos;
	}
	while (pos < nbr_spl);
}
...
#endif   // hiir_Downsampler2x16Avx512_CODEHEADER_INCLUDED
